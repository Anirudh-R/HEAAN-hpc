...
Scheme::Scheme(SecretKey& secretKey, Ring& ring, bool isSerialized ,bool isMaster) : ring(ring), isSerialized(isSerialized) ,isMaster(isMaster){
	addEncKey(secretKey);
...
void Scheme::addEncKey(SecretKey& secretKey) {
	...
	if(isMaster){
		/* only master process generates & stores key */
		if(isSerialized) {
...
void Scheme::addLeftRotKeys(SecretKey& secretKey) {
	...
	NTL_EXEC_RANGE(logN - 1, first, last);
...
void Scheme::evalExpAndEqual(Ciphertext& cipher, long logT, long logI) {
	...
	if(logSlots < logNh) {
		...
		for (long i = 0; i < logI + logT; ++i) {
		NTL_EXEC_RANGE(2, first, last);				//anirudhr
			for(long j = first; j < last; ++j){
				if(j == 0){
					squareAndEqual(c2);
