...
void Ring::encode(ZZ* mx, double* vals, long slots, long logp) {
	...
	long i, jdx, idx;
	//for (i = 0; i < slots; ++i) {
	NTL_EXEC_RANGE(slots, first, last);
	for (long i = first; i < last; ++i) {
		uvals[i].real(vals[i]);
	}
	NTL_EXEC_RANGE_END;
	...
	NTL_EXEC_RANGE(slots, first, last);
	for (long i = first, jdx = Nh + i*gap, idx = i*gap; i < last; ++i, jdx += gap, idx += gap) {
...
void Ring::decode(ZZ* mx, complex<double>* vals, long slots, long logp, long logq) {
	...
	NTL_EXEC_RANGE(slots, first, last);
	for (long i = first, idx = i*gap; i < last; ++i, idx += gap) {
		ZZ tmp;
		rem(tmp, mx[idx], q);
		...
		if (logSlots < logNh) {
			...
			for (ki = 0; ki < slots; ki += k) {
				for (pos = ki; pos < ki + k; ++pos) {
					...
					NTL_EXEC_RANGE(slots - pos, first, last);
					for (long i = first; i < last; ++i) {
						long deg;
					...
					NTL_EXEC_RANGE(pos, first, last);
					for (long i = first + slots - pos; i < last + slots - pos; ++i) {
						long deg;
					...
					NTL_EXEC_RANGE(dslots, first, last);
					for (long i = first, jdx = Nh + i*dgap, idx = i*dgap; i < last; ++i, jdx += dgap, idx += dgap) {
...
void Ring::multByMonomial(ZZ* res, ZZ* p, long monomialDeg) {
	...
	if (shift == 0) {
		...
		NTL_EXEC_RANGE(shift, first, last);
		for (long i = first; i < last; ++i) {
		...
		NTL_EXEC_RANGE(N - shift, first, last);
		for (long i = first + shift; i < last + shift; ++i) {
			res[i] = tmpx[i - shift];
		}
		NTL_EXEC_RANGE_END;
		delete[] tmpx;
...
void Ring::conjugate(ZZ* res, ZZ* p) {
	res[0] = p[0];
	//for (long i = 1; i < N; ++i) {
	NTL_EXEC_RANGE(N - 1, first, last);
	for (long i = first + 1; i < last + 1; ++i) {
		res[i] = -p[N - i];
	}
	NTL_EXEC_RANGE_END;
}
